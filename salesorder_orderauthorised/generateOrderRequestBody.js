function handler(params) {
  ...
  const sourceData = {
    ...
    purchase_order_number: data?.CustomerReference || data?.Order?.SaleOrderNumber || "",
    trackstar_tags: [data?.Order?.SaleOrderNumber, `cin7_id:${params.data?.var.saleId}`].filter(Boolean),
    ...
    line_items: (data?.Order?.Lines || []).map(orderLine => {
      const productMatch = products.find(p => p.sku === orderLine.SKU);
