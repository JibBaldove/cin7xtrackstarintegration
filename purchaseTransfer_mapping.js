function handler(params) {
      ...
      // Get inventory items if hasInventoryFields is true
      ...
      // Helper function to find lot info for a given SKU and warehouse_id
      const findLotInfo = (sku, warehouseId) => {
          if (!hasInventoryFields || !inventoryItemsMap[sku]) {
              return { lot_id: "", inventory_item_id: "", lot_expiration_date: null };
          }

          const inventoryItem = inventoryItemsMap[sku];
          const lots = inventoryItem.lots || [];

          // Find the first lot matching the warehouse_id with onhand > 0
          const matchingLot = lots.find(lot =>
              lot.warehouse_id === String(warehouseId) && lot.onhand > 0
          );

          // If no lot with onhand > 0, just find first lot matching warehouse_id
          const fallbackLot = lots.find(lot => lot.warehouse_id === String(warehouseId));

          const lot = matchingLot || fallbackLot;

          return {
              lot_id: lot?.lot_id || "",
              inventory_item_id: inventoryItem.id || "",
              lot_expiration_date: lot?.expiration_date ? ensureTimezone(lot.expiration_date) : null
          };
      };
      ...
      const sourceData = {
          ...
          boxes: lines.map(line => {
              const lotInfo = findLotInfo(line.SKU, warehouse_id);
              ...
              return {
                  line_items: [{
                      ...
                      inventory_item_id: lotInfo.inventory_item_id,
                      lot_expiration_date: lotInfo.lot_expiration_date
                  }],
